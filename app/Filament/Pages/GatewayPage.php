...
class GatewayPage extends Page
{
    ...
    public function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('SilverPay crie sua conta gratuitamente .')
                    ->description('')
                    ->schema([
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('create_account')
                ->label('Criar conta')
                ->url('https://silverpay.io/register')
                ->openUrlInNewTab()
                ->color('success'),
            Action::make('approve_account')
                ->label('Aprovar conta')
                ->url('https://api.whatsapp.com/send?phone=554699247838&text=Gostaria%20de%20aprovar%20minha%20conta.')
                ->openUrlInNewTab()
                ->color('info'),
        ];
    }
